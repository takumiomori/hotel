...
class RoomController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $category_name = Room::getCategoryName($category);

        if($items->count()>=$threshold){
            return redirect('/reservation/add')->with(['items'=>$items,'checkin'=>$checkin,'checkout'=>$checkout,'category_name'=>$category_name,]);
        }else{
